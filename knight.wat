(module
  ...
  (func $vec_clone (param $vec i32) (result i32)
        ...
        (local.set $data_size (i32.mul (local.get $len)
                                       (local.get $sizeof)))
        (local.set $new_vec (call $malloc (i32.add (i32.const 12) ;; cap, len, sizeof
                                                   (local.get $data_size))))
        (call $memcpy (call $__vec_data_ptr (local.get $new_vec))
  ...
  (func $__vec_maybe_realloc (param $vec i32) (result i32)
        ...
        (if (i32.gt_u (i32.add (local.get $len) (i32.const 1))
          ...
          (block
            ...
            (call $__vec_set_cap (local.get $vec)
                  (local.get $cap))
            (return
              (call $realloc (local.get $vec)
                    (i32.add (i32.const 12) ;; len, cap, sizeof
                             (i32.mul (local.get $cap)
                                      (local.get $sizeof))))))
          (return (local.get $vec)))
        (unreachable))

  ;; fn vec_push<T>(vec: Vec<T>) -> (Vec<T>, *mut T);
  ;;
  ;; Add an element to the back of the vector.
  ;;
  ;; The old vector is unconditionally invalidated, as this operation may
  ;; require reallocation.
  (func $vec_push (param $vec i32) (result i32 i32)
        (local $new_vec i32) (local $idx i32)
        (local.set $idx (call $__vec_get_len (local.get $vec)))
        (local.set $new_vec (call $__vec_maybe_realloc (local.get $vec)))
        (call $__vec_set_len (local.get $new_vec)
              (i32.add (i32.const 1)
                       (local.get $idx)))
        (call $vec_get_unchecked (local.get $new_vec) (local.get $idx))
        (local.get $new_vec))
  ...
  (func $main (export "_start")
        ...
        ;; (drop)
        ;; (local $buf i32)
        ;; (local.set $buf (call $malloc (i32.const 8)))
        ;; (call $dealloc (local.get $buf))
        (local $vec i32) (local $new_vec i32) (local $ptr i32)
        (local.set $vec (call $vec_create (i32.const 4)))
        (call $vec_push (local.get $vec))
        (local.set $new_vec)
        (local.set $ptr)
        (call $vec_destroy (local.get $new_vec)))
