(module
 ...
 ;;
 ;; struct VecInner<T> {
 ;;   /* 00 */ cap: usize, // how many elements this vector *could* store
 ;;   /* 04 */ len: usize, // how many elements this vector is storing
 ;;   /* 08 */ sizeof: usize, // if we had macros, we could monomorphize with each
 ;;                              type and have this statically. but alas, we don't,
 ;;                              so we won't.
 ;;   /* 12 */ data: *mut [T; cap],  // the raw data.
 ;; }
 ;; ```
 ...
 ;; fn __vec_set_cap<T>(vec: mut Vec<T>, cap: usize);
 ...
 ;; fn __vec_set_len<T>(vec: mut Vec<T>, len: usize);
 ...
 ;; fn __vec_set_sizeof<T>(vec: mut Vec<T>, sizeof: usize);
 (func $__vec_set_sizeof (param $vec i32) (param $sizeof i32)
       (i32.store offset=8 (local.get $vec) (local.get $sizeof)))
 
 ;; fn __vec_get_data_ptr<T>(vec: Vec<T>) -> *mut T;
 ;;
 ;; note: this ptr may not be valid
 (func $__vec_get_data_ptr (param $vec i32) (result i32)
       (i32.load offset=12 (local.get $vec)))
 
 ;; fn __vec_set_data_ptr<T>(vec: mut Vec<T>, ptr: *mut T);
 ;;
 ;; note: this ptr may not be valid
 (func $__vec_set_data_ptr (param $vec i32) (param $ptr i32)
       (i32.store offset=12 (local.get $vec) (local.get $ptr)))
 ...
 (func $vec_create (param $sizeof i32) (result i32)
       ...
       (call $__vec_set_data_ptr (local.get $ptr) (i32.const 0))
       (local.get $ptr))
 
 ;; fn vec_destroy<T>(vec: mut Vec<T>);
 ;;
 ;; NOTE: This does not call any type of destructor on the data within, but
 ;; will deallocate its backing memory.
 (func $vec_destroy (param $vec i32)
       (call $dealloc (local.get $vec)))
 
 ;; vec_clone<T>(vec: Vec<T>) -> Vec<T>;
 ;;
 ;; Clones a `Vec<T>`.
 (func $vec_clone (param $vec i32) (result i32)
       (local $data_size i32) (local $len i32) (local $sizeof i32)
       (local $data_ptr i32)
       (local $new_vec i32) (local $new_data i32)
       
       (local.set $len (call $__vec_get_len (local.get $vec)))
       (local.set $sizeof (call $__vec_get_sizeof (local.get $vec)))
       (local.set $data_size (i32.mul (local.get $len)
                                      (local.get $sizeof)))
       (local.set $data_ptr (call $__vec_get_data_ptr (local.get $vec)))
       
       (local.set $new_vec (call $malloc (i32.const 16)))
       (local.set $new_data (call $memdup
				  (local.get $data_ptr)
				  (local.get $data_size)))
       
       (call $__vec_set_len (local.get $new_vec) (local.get $len))
       (call $__vec_set_cap (local.get $new_vec) (local.get $len))
       (call $__vec_set_sizeof (local.get $new_vec) (local.get $sizeof))
       (call $__vec_set_data_ptr (local.get $new_vec) (local.get $new_data))
 ...
 (func $vec_get (param $vec i32) (param $idx i32) (result i32)
       ...
       (if (i32.ge_u (local.get $idx) (local.get $len))
           (block
            ...
            (unreachable)))
       (i32.add
        (call $__vec_get_data_ptr (local.get $vec))
        (i32.mul (call $__vec_get_sizeof (local.get $vec))
                 (local.get $idx))))
 
 ;; fn vec_get_unchecked<T>(vec: Vec<T>, idx: usize) -> *mut T;
 (func $vec_get_unchecked (param $vec i32) (param $idx i32) (result i32)
       (i32.add
        (call $__vec_get_data_ptr (local.get $vec))
        (i32.mul (call $__vec_get_sizeof (local.get $vec))
                 (local.get $idx))))

 ;; fn __vec_maybe_realloc<T>(vec: mut Vec<T>);
 (func $__vec_maybe_realloc (param $vec i32)
       (local $len i32) (local $cap i32) (local $sizeof i32)
       (local $data_ptr i32)
       (local $new_vec i32) (local $new_data i32)
       ...
       ;; if data_ptr is null, alloc
       (if (i32.eqz (local.get $data_ptr))
	   (block
	    (call $__vec_set_cap (local.get $vec) (i32.const 1))
	    (call $__vec_set_data_ptr (local.get $vec)
		  (call $malloc (local.get $sizeof)))
	    (local.set $cap (i32.const 1))))
       
       ;; if len + 1 > cap, realloc
       (if (i32.gt_u (i32.add (local.get $len) (i32.const 1))
                     (local.get $cap))
           (block
            (local.set $cap (i32.mul (i32.const 2)
                                     (i32.add (i32.const 1)
                                              (local.get $cap))))
            (call $__vec_set_cap (local.get $vec)
                  (local.get $cap))
	    (call $__vec_set_data_ptr (local.get $vec)
		  (call $realloc
			(local.get $data_ptr)
 ...
 ;; fn vec_push<T>(vec: mut Vec<T>) -> *mut T;
 ;;
 ;; Add an element to the back of the vector.
 (func $vec_push (param $vec i32) (result i32)
       (local $idx i32)
       
       (local.set $idx (call $__vec_get_len (local.get $vec)))
       (call $__vec_maybe_realloc (local.get $vec))
       (call $__vec_set_len (local.get $vec)
             (i32.add (i32.const 1)
                      (local.get $idx)))
       (call $vec_get_unchecked (local.get $vec) (local.get $idx)))
 
 (func $vec_len (param $vec i32) (result i32)
       (call $__vec_get_len (local.get $vec)))
 
 ;; fn vec_pop<T>(vec: Vec<T>) -> bool;
 ;;
 ;; Pop an element from the back of the vector. Returns true if an element
 ;; was actually removed.
 (func $vec_pop (param $vec i32) (result i32)
       (local $len i32)
       (local.set $len (call $__vec_get_len (local.get $vec)))
 ...
 (func $main (export "_start")
       (local $vec i32) (local $ptr i32) (local $arg i32)
...
;;DATA BEGIN;;
 ...
 (global $data_str3_size i32 (i32.const 0x14))
 (data (i32.const 0x9c) "Hello, world! %u %u\n")

 ;; name: str4
 ;; size: 0x0b
 (global $data_str4_offset i32 (i32.const 0xb0))
 (global $data_str4_size i32 (i32.const 0x0b))
 (data (i32.const 0xb0) "dropped %u\n")

 ;; name: garbage_u32
 ;; size: 0x04
 (global $data_garbage_u32_offset i32 (i32.const 0xbc))
 (global $data_garbage_u32_size i32 (i32.const 0x04))
 (data (i32.const 0xbc) "\00\00\00\00")

 ;; name: static_ciovec
 ;; size: 0x08
 (global $data_static_ciovec_offset i32 (i32.const 0xc0))
 (global $data_static_ciovec_size i32 (i32.const 0x08))
 (data (i32.const 0xc0) "\00\00\00\00\00\00\00\00")

 ;; name: garbage_v128
 ;; size: 0x10
 (global $data_garbage_v128_offset i32 (i32.const 0xc8))
 (global $data_garbage_v128_size i32 (i32.const 0x10))
 (data (i32.const 0xc8) "\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00\00")

 (global $data_end i32 (i32.const 0xd8))
...
;;DATA END;;
 ...
 (export "vec_create" (func $vec_create))
 (export "vec_destroy" (func $vec_destroy))
 (export "vec_clone" (func $vec_clone))
